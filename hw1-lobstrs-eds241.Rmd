...
We cannot investigate a true counterfactual of either site, as we cannot observe an instance of either site not as MPAs at the time frame of this experiment. The next best realistic counterfactual are non-MPA sites in the same area as our MPA sites. Researches could have chosen sites as close as possible to our treatment sites, but ecologically we know processes like spillover effects would not make these control sites independent of our treatment. Likewise, the control sites themselves are not close enough together for these ecological processes to eliminate independence. The control sites are far enough from the treatments and each other, but not so far they are experiencing different oceanographic conditions to deam them incomparable, so I would argue these sites provide a sufficient comparison under ceteris paribus.
...
spiny_counts %>% 
    ggplot(aes(colour = mpa, x = mean_size)) +
    geom_density()
```
...
Based on our model, an MPA reef will have an estimated 23 spiny lobsters, whereas a non-MPA reef will have an estimate of only 18 spiny lobsters.
...
The residuals are the difference between the true value and fitted value, which ideally is very close to 0. An assumption of an OLS model is that are data are linear and normally distributed. Because our residuals fall far from 0 (the normal distribution quantiles), we know our data do not satisfy these assumptions and are not normally distributed.
...
Our model predicts an estimated average of `r change_pois` % more lobsters in MPA reefs compared to non-MPA reefs.

In a poisson distribution, the variance equals the mean, both repressented by the model's sole parameter $\lambda$. Dispersion means the variance = mean, which is a perfect fit and an assumption of the model. Overdispersion occurs when the observed variance \> mean, which often happens with data that are "clumped", similar to zero-inflation where data fall closely in one area of the distribution instead of normally, or if important predictors are missing in the model.

|           | OLS                 | Poisson              |
|-----------|---------------------|----------------------|
| Treatment p-value | `r treat_p_ols`     | `r treat_p_pois`     |

The p-value goes from `r treat_p_ols` to `r treat_p_pois` when changing the model from an OLS to poisson. This is a change from a insignificant effect of mpa status on lobster abundance, to a significant effect just based on the statistical model. If we look at the model assumptions and they seem appropriate for a poisson, the p-value is more likely to be accurate.
...
ggplot(plot_counts, aes(x = year, y = counts)) +
...
extracred_counts <- tidyextracred %>% 
    ...
    summarize(counts = sum(count, na.rm = TRUE),
              mean_size = mean(size_mm, na.rm = TRUE)) %>% 
    mutate(treat = case_when(site == "IVEE" ~ "MPA",
                             site == "NAPL" ~ "MPA",
                             site == "MOHK" ~ "non_MPA",
                             site == "CARP" ~ "non_MPA",
                             site == "AQUE" ~ "non_MPA"))
...
```{r}
extracred_counts %>% 
    ggplot(aes(colour = treat, x = mean_size)) +
    geom_density()
```
...
export_summs(m1_extra, m2_extra, m3_extra,
             model.names = c("OLS","Poisson", "NB"),
             statistics = "none")
